...
const { t } = useI18n();
const rechargeList = computed(() => walletStore.rechargeList);
const getWalletDataLoading = ref(false);
const selectedCoin = ref('');
const selectedType = ref('');
const activeTypeArray = computed(() => walletStore.netList[selectedCoin.value]);
const currentAddress = computed(
  () => walletStore.netAddress[selectedType.value]
);
...
const coinItemClick = async (coin) => {
  if (coin !== selectedCoin.value) {
    ...
    selectedType.value = '';
    await walletStore.getNetList(coin);
    await promiseTimeout(50);
    selectedType.value = activeTypeArray.value[0]?.chain;
  }
};
watch(selectedType, async (nv) => {
  if (nv) {
    await walletStore.getNetAddress(nv);
  }
});
